...
def add_to_cart(user_id, product_id):
    try:
        ...
        # Log the attempt
        logger.info(f"Adding to cart: user_id={user_id}, product_id={product_id}")
        ...
        if existing:
            logger.info(f"Updating existing cart item: quantity={existing[0]} -> {existing[0]+1}")
            c.execute("UPDATE cart SET quantity = quantity + 1 WHERE user_id = ? AND product_id = ?",
                      (user_id, product_id))
        else:
            logger.info(f"Inserting new cart item")
            c.execute("INSERT INTO cart (user_id, product_id, quantity) VALUES (?, ?, 1)",
                     (user_id, product_id))
        ...
        c.execute("SELECT quantity FROM cart WHERE user_id = ? AND product_id = ?", 
                  (user_id, product_id))
        result = c.fetchone()
        logger.info(f"Verification: cart now has quantity={result[0] if result else 'NOT FOUND'}")
...
def debug_cart(message):
    """Debug command to check cart state"""
    user_id = message.from_user.id
    ...
    try:
        ...
        debug_msg += f"*Tables:* {', '.join([t[0] for t in tables])}\n"
        debug_msg += f"*Products in DB:* {product_count}\n"
        debug_msg += f"*Cart Items:* {len(cart_items)}\n\n"
        
        if cart_items:
            debug_msg += "*Your cart entries:*\n"
            for item in cart_items:
                debug_msg += f"  User: {item[0]}, Product: {item[1]}, Qty: {item[2]}\n"
        else:
            debug_msg += "Cart is empty in database.\n"
...
def handle_query(call):
    ...
    try:
        ...
        # Product actions
            ...
            if product:
                success = add_to_cart(user_id, product_id)
